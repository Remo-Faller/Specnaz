#! /usr/bin/env python3
# Autor @Remo-Faller
# Ostatnia Aktualizacja: 13/12/2024 - version 1.5
    ...
    if len(arg) == 1: menu()
        ...
        if arg[1] in ("update"):
            if which("termux-setup-storage"): path = "$PREFIX/bin/specnaz"
                ...
                if os.path.isdir("/usr/bin/"): path = "/usr/bin/specnaz"
                else: path = "/usr/bin/specnaz"
            os.system(f"wget https://raw.githubusercontent.com/Remo-Faller/Specnaz/main/specnaz.py -O {path} && chmod +x {path}")
